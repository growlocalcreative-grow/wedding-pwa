...
export default function AdminPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    let table = activeTab === 'tasks' ? 'tasks' : activeTab === 'resources' ? 'resources' : 'diy_projects';
    const payload: any = activeTab === 'tasks' ? { title, directions: desc, phase_id: parseInt(val1), assigned_user_id: val2 || null, estimated_cost: parseFloat(cost) } : activeTab === 'resources' ? { name: title, notes: desc, category: val1, phone, email, price_quoted: parseFloat(cost) } : { title, materials_list: desc, estimated_cost: parseFloat(cost), assigned_user_id: val2 || null, strategy: 'DIY' };
    ...
    if (!error) {
      ...
      await supabase.from('activity_log').insert([{
        message: `Admin ${actionLabel} a ${activeTab.slice(0,-1)}: ${title}`,
        user_id: currentUser.id
      }]);

      setEditingId(null); setTitle(''); setDesc(''); setCost('0'); setPhone(''); setEmail(''); fetchData();
    }
  }
  ...
  function startEdit(item: any) {
    ...
    setPhone(item.phone || ''); setEmail(item.email || '');
  ...
  async function approveTask(task: any) {
    ...
    await supabase.from('activity_log').insert([{
      message: `Bride approved: ${task.title} 🥂`,
      user_id: currentUser.id
    }]);
    fetchData();
  }
  ...
  return (
    <main className={`min-h-screen pb-32 font-sans text-slate-800 transition-colors duration-500 ${theme.bg}`}>
      <header className={`bg-white border-b sticky top-0 z-20 shadow-sm flex flex-col items-center p-6 transition-all duration-500 ${theme.border}`}>
        <h1 className="text-xl font-serif italic tracking-tight text-slate-800 leading-none">Bride&apos;s Desk</h1>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-8">
        <section className={`card-wedding border-2 bg-white p-6 shadow-md transition-all duration-500 ${theme.border}`}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {activeTab === 'tasks' && <select className="w-full p-3 rounded-xl border border-slate-200 text-sm bg-white" value={val1} onChange={(e) => setVal1(e.target.value)}>{[1,2,3,4].map(n => <option key={n} value={n}>Phase {n}</option>)}</select>}
              {activeTab === 'resources' && <input type="text" placeholder="Category" className="w-full p-3 rounded-xl border border-slate-200 text-sm outline-none" value={val1} onChange={(e) => setVal1(e.target.value)} />}
            ...
            <button type="submit" className={`w-full text-white font-bold py-3 rounded-xl text-[10px] uppercase tracking-widest shadow-md ${theme.btn}`}>{editingId ? 'Save Changes' : `Confirm Bite`}</button>
          </form>
        </section>
          ...
          {items.map((item) => (
            <div key={item.id} className="card-wedding py-4 px-5 flex justify-between items-center shadow-sm bg-white/80 border-l-4 border-l-slate-200">
              <div className="flex-1 pr-4">
                <h3 className="text-sm font-bold text-slate-700">{item.title || item.name}</h3>
                <p className="text-[9px] text-slate-400 mt-1 uppercase font-bold tracking-tighter italic">
                   {item.status === 'pending_review' ? '⚠️ NEEDS APPROVAL' : `Value: $${item.estimated_cost || item.price_quoted || 0}`}
                </p>
              </div>
              <div className="flex gap-2">
                {item.status === 'pending_review' && <button onClick={() => approveTask(item)} className="bg-emerald-500 text-white px-3 py-1 rounded-lg text-[9px] font-bold uppercase shadow-sm">Approve</button>}
                <button onClick={() => startEdit(item)} className="p-2 bg-slate-50 rounded-lg text-sm active:scale-90 transition-all">✏️</button>
                <button onClick={() => handleDelete(item.id)} className="p-2 bg-rose-50 rounded-lg text-sm active:scale-90 transition-all text-rose-400">🗑️</button>
              </div>
            </div>
          ))}
        </section>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 h-16 flex items-center justify-around px-6 z-50">
        <Link href="/" className="text-slate-400 flex flex-col items-center hover:scale-105 transition-all"><span className="text-xl">🏠</span><span className="text-[9px] font-bold mt-1 tracking-tight text-center">Exit</span></Link>
      </nav>
    </main>
  );
}