...
export default function TasksPage() {
  ...
  async function fetchData() {
    ...
    // Fetch all phases and all tasks
  ...
  // --- THE IMPROVED GATING LOGIC (Fixes Bug 1 & 3) ---
  const isPhaseUnlocked = (phaseStep: number) => {
    if (phaseStep === 1) return true; // Phase 1 is always open
    
    // Check ALL tasks in ALL phases that come BEFORE this one
    // e.g., if checking Phase 4, look at 1, 2, and 3.
    const allPreviousTasks = tasks.filter(t => t.phase_id < phaseStep);
    
    // If there are no tasks at all in previous phases, it's unlocked by default
    if (allPreviousTasks.length === 0) return true; 

    // ALL tasks in previous phases MUST be 'completed' (Approved by Admin)
  ...
  async function completeTask(taskId: number) {
    ...
    if (!error) {
      // Update local state so button changes to "REVIEWING"
      setTasks(tasks.map(t => t.id === taskId ? { ...t, status: 'pending_review' } : t));
    }
  }

  if (loading) return <div className="p-10 text-center italic text-rose-300 font-serif">Checking the Elephant...</div>;
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24 font-sans text-slate-800">
      <div className="bg-white border-b border-rose-100 p-6 sticky top-0 z-10 shadow-sm text-center">
        ...
        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-2 italic">Phase by Phase Logic</p>
      </div>

      <div className="max-w-2xl mx-auto p-4 space-y-10">
        {phases.map((phase) => {
          const unlocked = isPhaseUnlocked(phase.step_number);
          // Only show tasks assigned to THIS user OR "Everyone" (null)
          const phaseTasks = tasks.filter(t => 
            t.phase_id === phase.step_number && 
            (t.assigned_user_id === userId || t.assigned_user_id === null)
          );
            ...
            <section key={phase.id} className={`transition-all duration-500 ${!unlocked ? 'opacity-30 pointer-events-none' : ''}`}>
              <div className="flex items-center justify-between mb-4 px-2">
                <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                  Phase {phase.step_number}: {phase.name}
                </h2>
              ...
              {unlocked ? (
                <div className="space-y-3">
                    ...
                    <div key={task.id} className={`card-wedding relative overflow-hidden ${task.is_bro_deal ? 'bro-deal-card' : ''} border-l-4 ${task.status === 'completed' ? 'border-l-emerald-400' : 'border-l-rose-200'}`}>
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex-1">
                          <h3 className={`font-bold text-sm ${task.status === 'completed' ? 'text-slate-300 line-through' : 'text-slate-800'}`}>
                            {task.title}
                          </h3>
                          <p className="text-[11px] text-slate-500 mt-1 italic leading-relaxed">"{task.directions}"</p>
                          
                          {/* ADMIN COMMENTS (Bug Fix Round 2) */}
                          {task.admin_comments && (
                            <div className="mt-3 p-2 bg-rose-50/50 rounded-lg border border-rose-100">
                              <p className="text-[9px] font-bold text-rose-400 uppercase tracking-tighter italic">Note from Bride:</p>
                              <p className="text-[11px] text-rose-700 italic mt-0.5 leading-snug">{task.admin_comments}</p>
                            </div>
                          )}
                        </div>
                        ...
                        {task.status === 'todo' && (
                          <button 
                            ...
                            className="bg-rose-100 text-rose-600 font-bold py-2 px-3 rounded-xl text-[10px] shadow-sm active:scale-95 transition-all"
                          >
                            DONE
                          </button>
                        )}
                        {task.status === 'pending_review' && (
                          <span className="bg-sky-50 text-sky-500 text-[9px] font-bold px-3 py-2 rounded-xl italic animate-pulse">
                            REVIEWING
                          </span>
                        )}
                        {task.status === 'completed' && (
                          <span className="text-emerald-500 font-bold text-[10px] bg-emerald-50 px-2 py-1 rounded">
                            ✅ APPROVED
                          </span>
                        )}
                      </div>
                  ...
                  {phaseTasks.length === 0 && (
                    <p className="text-[10px] text-slate-300 italic text-center py-2">No tasks assigned to you here.</p>
                  )}
                </div>
              ) : (
                <div className="card-wedding bg-slate-100/30 border-dashed border-slate-200 py-6 text-center shadow-inner">
                  <p className="text-[10px] text-slate-400 font-medium italic">Finish all previous phases to unlock. 🐘</p>
                </div>
              )}
            </section>
          );
        ...
        <section className="mt-12 mb-8 p-6 bg-white rounded-2xl border-2 border-dashed border-slate-200">
          <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center mb-4 italic">Suggest a Task for the Bride</h3>
          <div className="flex gap-2">
            <input 
                type="text" 
                id="suggestInput" 
                placeholder="e.g. Rent a generator" 
                className="flex-1 p-3 rounded-xl border border-slate-200 text-sm outline-none focus:ring-1 focus:ring-rose-200" 
            />
            <button 
              onClick={async () => {
                ...
                await supabase.from('tasks').insert([{
                  ...
                  directions: 'Suggested by ' + (userId ? 'User' : 'Team'), 
                  phase_id: 1, 
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 h-16 flex items-center justify-around px-6 z-50">
        ...
        <Link href="/tasks" className="text-rose-500 flex flex-col items-center hover:scale-105 active:scale-90 transition-all">
          <span className="text-xl">✅</span>
          <span className="text-[9px] font-bold mt-1 underline decoration-2 underline-offset-4 decoration-rose-200">Tasks</span>
        </Link>
        <Link href="/workshop" className="text-slate-400 flex flex-col items-center hover:scale-105 active:scale-90 transition-all">
          <span className="text-xl">🎨</span>
          <span className="text-[9px] font-bold mt-1">DIY</span>
        </Link>
